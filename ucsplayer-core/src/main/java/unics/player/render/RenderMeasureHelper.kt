...
internal class RenderMeasureHelper {
    ...
    fun doMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (aspectType == AspectRatioType.MATCH_PARENT) {
            ...
            plogi2("RenderMeasureHelper") {
                val sb = StringBuilder("doMeasure -> ").appendLine()
                sb.append("aspectType=").appendLine(aspectType)
                sb.append("mMeasuredWidth=").appendLine(measuredWidth)
                sb.append("mMeasuredHeight=").appendLine(measuredHeight)
                sb.toString()
            }
            return
        }
...
//        val heightSpecMode = View.MeasureSpec.getMode(relateHeightMeasureSpec)
        ...
        if (isWiderDisplay) { //通过控件能够分配的比例跟图像比例做了比较，因此不再需要判断测量Mode
            ...
            when (aspectType) {
                ...
                AspectRatioType.SCALE_ORIGINAL -> {
                    preMeasuredHeight = min(heightSpecSize, videoHeight)
                    preMeasuredWidth = (preMeasuredHeight * displayAspectRatio).toInt()
                }
                else -> { //缩放模式
                    preMeasuredHeight = heightSpecSize
        ...
        plogi2("RenderMeasureHelper") {
            val sb = StringBuilder("doMeasure -> ").appendLine()
            sb.append("aspectType=").appendLine(aspectType)
            sb.append("isWiderDisplay=").appendLine(isWiderDisplay)
            sb.append("displayAspectRatio=").appendLine(displayAspectRatio)
            sb.append("videoWidth=").appendLine(videoWidth)
            sb.append("videoHeight=").appendLine(videoHeight)
            sb.append("widthSpecSize=").appendLine(widthSpecSize)
            sb.append("heightSpecSize=").appendLine(heightSpecSize)
            sb.append("mMeasuredWidth=").appendLine(measuredWidth)
            sb.append("mMeasuredHeight=").appendLine(measuredHeight)
            sb.toString()
        }
